...
public final class MolecularAssemblerMenu extends AbstractContainerMenu {
    ...
    public MolecularAssemblerMenu(int id, Inventory inventory, FriendlyByteBuf buffer) {
        this(id, inventory, (MolecularAssemblerBlockEntity) inventory.player.level()
                .getBlockEntity(buffer.readBlockPos()), new SimpleContainerData(5));
    }
    public MolecularAssemblerMenu(int id, Inventory inventory,
            MolecularAssemblerBlockEntity blockEntity, ContainerData data) {
        super(ModMenus.MOLECULAR_ASSEMBLER_9X9.get(), id);
        this.blockEntity = blockEntity;
        this.data = data;
        for (int row = 0; row < 9; row++) for (int column = 0; column < 9; column++) {
            addSlot(new SlotItemHandler(blockEntity.getItems(), column + row * 9,
                    8 + column * 18, 18 + row * 18));
        }
        addSlot(new SlotItemHandler(blockEntity.getItems(), MolecularAssemblerBlockEntity.OUTPUT_SLOT, 190, 82) {
        ...
        for (int column = 0; column < 9; column++) addSlot(new Slot(inventory, column, 8 + column * 18, y + 58));
        addDataSlots(data);
    }
    @Override public boolean stillValid(@NotNull Player player) {
        return stillValid(net.minecraft.world.inventory.ContainerLevelAccess.create(
                blockEntity.getLevel(), blockEntity.getBlockPos()), player, ModBlocks.MOLECULAR_ASSEMBLER_9X9.get());
    }
    @Override public boolean clickMenuButton(@NotNull Player player, int id) {
        if (id == UNLOCK_BUTTON_ID) { blockEntity.clearRecipeLock(); return true; }
        return false;
    }
    @Override public @NotNull ItemStack quickMoveStack(@NotNull Player player, int index) {
        Slot slot = slots.get(index);
        if (!slot.hasItem()) return ItemStack.EMPTY;
        ItemStack original = slot.getItem(); ItemStack copy = original.copy();
        ...
        } else if (!moveItemStackTo(original, 0, MolecularAssemblerBlockEntity.GRID_SLOTS, false)) return ItemStack.EMPTY;
        if (original.isEmpty()) slot.set(ItemStack.EMPTY); else slot.setChanged();
        slot.onTake(player, original); return copy;
    }
    public int getProgressPixels(int width) { return data.get(1) <= 0 ? 0 : Math.min(width, data.get(0) * width / data.get(1)); }
    public int getEnergyPixels(int height) { return data.get(3) <= 0 ? 0 : Math.min(height, data.get(2) * height / data.get(3)); }
